...
import { actPostDataTypesetting } from '../../modules/Setting';
const FormAdd = () => {
  /**
  ...
  const { MODAL_ADD_TYPESETTING } = modals;
  ...
  const validation = useFormik({
    ...
    onSubmit: (values) => {
      ...
      dispatch(actPostDataTypesetting(values));
    },
  });
  ...
  return (
    <Modal isOpen={MODAL_ADD_TYPESETTING}>
      <Form
        onSubmit={(e) => {
          e.preventDefault();
      ...
      >
        <div className='modal-header d-flex justify-content-between'>
          <h5 className='modal-title'>Create Type Setting</h5>
        </div>
        <ModalBody>
          {/* Field name */}
          <FormGroup>
        ...
        <ModalFooter>
          ...
          <Button
            ...
            onClick={() => dispatch(__openModal({ modal: 'MODAL_ADD_TYPESETTING', open: false }))}
          >
            Close
          </Button>
          {/* Button submit form */}
